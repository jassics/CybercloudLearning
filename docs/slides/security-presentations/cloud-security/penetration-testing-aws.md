...
- **AWS's rules for pentesting:** no DoS/DDoS, port flooding, protocol flooding, or network stress testing without prior notification to AWS - 8 services are pre-permitted for testing (EC2, RDS, CloudFront, Aurora, API Gateway, Lambda, Lightsail, Elastic Beanstalk); anything else requires explicit authorization via `aws-security-simulated-event@amazon.com`.
